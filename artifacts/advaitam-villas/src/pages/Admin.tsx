...
export default function Admin() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case "new": return <Badge className="bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 border-blue-500/20">New</Badge>;
      case "contacted": return <Badge className="bg-amber-500/10 text-amber-500 hover:bg-amber-500/20 border-amber-500/20">Contacted</Badge>;
      case "qualified": return <Badge className="bg-green-500/10 text-green-500 hover:bg-green-500/20 border-green-500/20">Qualified</Badge>;
      case "lost": return <Badge className="bg-red-500/10 text-red-500 hover:bg-red-500/20 border-red-500/20">Lost</Badge>;
  ...
  const getSourceBadge = (source: string) => {
    switch(source) {
      case "brochure": return <Badge variant="outline" className="bg-purple-500/10 text-purple-400 border-purple-500/20">Brochure</Badge>;
      case "site-visit": return <Badge variant="outline" className="bg-teal-500/10 text-teal-400 border-teal-500/20">Site Visit</Badge>;
      case "whatsapp": return <Badge variant="outline" className="bg-green-500/10 text-green-400 border-green-500/20">WhatsApp</Badge>;
      case "exit-popup": return <Badge variant="outline" className="bg-orange-500/10 text-orange-400 border-orange-500/20">Exit Popup</Badge>;
      ...
      <header className="border-b border-border bg-card">
        <div className="container mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-serif text-xl font-bold tracking-wide">ADVAITAM SALES</span>
          </div>
      ...
      <main className="flex-1 container mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="bg-card border-border shadow-sm">
            ...
            <CardContent>
              {isLoadingStats ? <Skeleton className="h-8 w-20" /> : (
                <div className="text-3xl font-bold text-green-500">{stats?.byStatus.qualified || 0}</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border shadow-sm">
            ...
            <CardContent>
              {isLoadingStats ? <Skeleton className="h-8 w-20" /> : (
                <div className="text-3xl font-bold text-red-500">{stats?.byStatus.lost || 0}</div>
              )}
            </CardContent>
          </Card>
        </div>
            ...
            <Table>
              <TableHeader className="bg-secondary/30">
                <TableRow className="hover:bg-transparent border-border">
                  <TableHead className="w-[200px] text-muted-foreground font-medium">Contact</TableHead>
              ...
              <TableBody>
                {isLoadingLeads ? (
                  ...
                  filteredLeads.map((lead) => (
                    <TableRow key={lead.id} className="border-border hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => handleEditClick(lead)}>
                      <TableCell>
                      ...
                      <TableCell>
                        ...
                        {lead.notes && (
                          <div className="text-xs text-muted-foreground mt-1 truncate max-w-[200px] flex items-center gap-1.5">
                            <MessageSquare className="w-3 h-3 shrink-0" /> {lead.notes}
                          </div>
                        )}
                      </TableCell>
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the lead from our database.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
